...
def load_and_prepare_samples(input_dir: str) -> list:
...
def normalize_weather_variables(
    ...
    for var in weather_vars:
        ...
        print(f"  {var:12s} ({transform_type:20s}): mean={mean_val:10.4f}, std={std_val:10.4f}")
...
def main():
    ...
    if train is not None:
        train.to_zarr(os.path.join(OUTPUT_DIR, "train.zarr"), mode="w")
    if val is not None:
        val.to_zarr(os.path.join(OUTPUT_DIR, "validation.zarr"), mode="w")
    if test is not None:
        test.to_zarr(os.path.join(OUTPUT_DIR, "test.zarr"), mode="w")
